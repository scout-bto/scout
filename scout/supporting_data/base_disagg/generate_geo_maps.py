...
def process_end_use_energy(sector, filedir, filename, weathers, mymap, scoutgeo_df, geos, outdir):
...
def process_end_use_stock(sector, filedir, filename, weathers, mymap, scoutgeo_df, geos, outdir):
...
def process_tech_energy(sector, filedir, filename, weathers, mymap, scoutgeo_df, geos, outdir, fdir):
    """Process technology-level energy data (placeholder for now)."""
    print(f"  Tech energy processing for {sector} (not yet implemented)")
    pass

def process_tech_stock(sector, filedir, filename, weathers, mymap, scoutgeo_df, geos, outdir, fdir):
...
def main():
    parser = argparse.ArgumentParser(description="Generate geographic disaggregation maps from ResStock and ComStock data.")
    parser.add_argument('--weather-year', type=str, default='amy2018', help='Weather year to process (e.g., amy2018, tmy3).')
    parser.add_argument('--comstock-path', type=str, default='input/2024_comstock', help='Path to ComStock data directory.')
    parser.add_argument('--resstock-path', type=str, default='input/2024_resstock', help='Path to ResStock data directory.')
    parser.add_argument('--output-dir', type=str, default='output', help='Directory to save the output CSV files.')
    parser.add_argument('--all', action='store_true', help='Generate all output files (default behavior).')
    parser.add_argument('--force', action='store_true', help='Overwrite existing output files.')
    parser.add_argument('--install', action='store_true', help='Copy the generated files to the scout/supporting_data/convert_data/geo_map directory.')
    ...
    if not os.path.exists(resstock_data_path):
        ...
        print("  To generate residential disaggregation maps, provide BuildStock parquet files.")
    else:
