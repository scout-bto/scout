...
class AEOInputRegistry:
    ...
    The "raw" mode corresponds to the AEO-delivered files staged in
    ``inputs/`` before preprocessing. The "processed" mode corresponds to
    the files Scout modules read after preprocessing. Not every file in the
    processed set is generated by ``scout.eia_file``; several are raw AEO
    inputs that Scout reads directly from ``inputs/``.
    """

    RAW = {
        "res_db_source": "RDM_DBOUT.txt",
        "res_dgen_source": "RDM_DGENOUT.txt",
    ...
    INTERNAL = {
        "res_db_backup": "RDM_DBOUT-orig.txt",
    }
    ...
    def path_map(cls, mode: str, input_dir: Path = None):
        ...
        base = input_dir if input_dir is not None else FilePaths.INPUTS
        files = cls.files_for_mode(mode)
        return {k: base / v for k, v in files.items()}
